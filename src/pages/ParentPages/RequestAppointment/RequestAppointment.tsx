...
//   const [availabilityData, setAvailabilityData] = useState<SlotAvailabilityResponse>({});
//   const [availabilityLoading, setAvailabilityLoading] = useState<boolean>(false);

//   // --- SEARCH & PAGINATION state for Requested Booking table ---
//   const [reqSearch, setReqSearch] = useState<string>("");
//   const [reqSearchImmediate, setReqSearchImmediate] = useState<string>("");
//   const [reqPage, setReqPage] = useState<number>(1);
//   const [reqPageSize, setReqPageSize] = useState<number>(DEFAULT_PAGE_SIZE);
//   const reqSearchTimer = useRef<ReturnType<typeof setTimeout> | null>(null);

//   // Fetch slot availability for visible month
//   const fetchSlotAvailability = useCallback(async (y: number, m: number) => {
//     setAvailabilityLoading(true);
//     try {
//       const token = localStorage.getItem("patient-token");
//       const queryYear = y;
//       const queryMonth = m + 1; // month is 0-based, API expects 1-based
//       const url = `${API_BASE_URL}/api/parent/slot-availability?year=${queryYear}&month=${queryMonth}`;
...
//       // const json: SlotAvailabilityResponse = await res.json();
//       // console.log(json);
//       // setAvailabilityData(json || {});
//       const json = await res.json();
// setAvailabilityData(json.days || {});
//     } catch (err: any) {
//       setAvailabilityData({});
//       // Only show a toast in production; dev may not have endpoint
//       // toast.error("Failed to fetch slot availability");
//     }
//     setAvailabilityLoading(false);
//   }, []);

//   // Load availabilities for this month whenever month or year changes
//   useEffect(() => {
//     fetchSlotAvailability(year, month);
//   }, [year, month, fetchSlotAvailability]);

//   // Bookings and master data logic remains mostly same
//   const getFilteredRequestedBookings = useCallback(() => {
//     if (!reqSearch) return requestedBookings;
//     const term = reqSearch.toLowerCase();
//     return requestedBookings.filter(br => {
...
//   useEffect(() => {
//     // Debounce: only set reqSearch after pause
//     if (reqSearchTimer.current) clearTimeout(reqSearchTimer.current as any);
//     reqSearchTimer.current = setTimeout(() => {
//       setReqSearch(reqSearchImmediate.trim());
//     }, 300);
...
//   // Add session for selected date ONLY if at least one slot is available that day
//   const addSessionForDate = (day: number) => {
//     const dateKey = getDateKey(year, month + 1, day);
//     // Only allow adding session if availability exists
//     const slotsObj = availabilityData[dateKey]?.slots;
//     const hasAvailableSlots = slotsObj
//       ? Object.values(slotsObj).some((s) => s.available > 0)
//       : true; // If no data, fallback to old logic
//     if (
//       typeof maxSelectableDates === "number" &&
//       sessions.length >= maxSelectableDates
//     ) {
//       return;
//     }
//     if (slotsObj && !hasAvailableSlots) return;
//     setSessions((prev) => [...prev, { date: dateKey, slotId: "" }]);
//   };
...
//   // New: Enforce slot available count in updateSlotId
//   const updateSlotId = (idx: number, slotId: string) => {
//     const session = sessions[idx];
//     if (!session) return;
//     const chosenForDate = sessions
...
//       return;
//     }
//     // Check slot availability for this day/slot. If 0 available, block and toast.
//     const slotAvail = availabilityData[session.date]?.slots?.[slotId];
//     if (slotId && slotAvail && slotAvail.available <= 0) {
//       toast.error("Slot not available. Please pick another slot.");
//       return;
//     }
//     setSessions((prev) =>
//       prev.map((s, i) => (i === idx ? { ...s, slotId } : s))
...
//   // Also prevent booking if any chosen slot is actually unavailable
//   const allSlotsAvailable =
//     sessions.length === 0
//       ? true
//       : sessions.every((s) => {
//           if (!s.date || !s.slotId) return false;
//           const avail = availabilityData[s.date]?.slots?.[s.slotId];
//           // Limited case: only check avail if it exists, otherwise assume available
...
//   // Tooltip state for day hover — shows per-slot breakdown
//   const [hoveredDay, setHoveredDay] = React.useState<string | null>(null);

//   // Availability bar width helper (capped 0–100%)
//   const availPct = (avail: number, capacity: number) =>
//     capacity > 0 ? Math.round(Math.max(0, Math.min(100, (avail / capacity) * 100))) : 0;
...
//               </span>
//               {availabilityLoading && (
//                 <span className="ml-2 text-[11px] text-sky-500 animate-pulse font-medium">
...
//                 className="p-1.5 rounded-lg border border-slate-200 bg-white hover:bg-blue-50 hover:border-blue-300 transition text-slate-600"
//                 type="button"
//               >
...
//             {DAYS.map((d) => (
//               <div key={d} className="py-2 text-center text-[11px] font-semibold tracking-widest text-slate-400 uppercase">
//                 {d}
//               </div>
//             ))}
//           </div>
...
//           <div className="grid grid-cols-7">
//             {Array.from({ length: getStartDay(year, month) }).map((_, i) => (
//               <div key={`empty-${i}`} className="h-28 border-b border-r border-slate-100" />
//             ))}
...
//                 typeof maxSelectableDates === "number" &&
//                 sessions.length >= maxSelectableDates;
//               const availForDay = availabilityData[dateKey];
//               const hasAvailInfo = !!availForDay;
//               const allSlotsBooked =
//                 hasAvailInfo &&
//                 availForDay.slots &&
//                 Object.values(availForDay.slots).length > 0 &&
//                 Object.values(availForDay.slots).every((s) => s.available === 0);
//               const dayIsSelectable = !isAtMax && !(hasAvailInfo && allSlotsBooked);

//               // Colour-code availability
//               const normalAvail = availForDay?.summary?.normalAvailable ?? 0;
//               const limitedAvail = availForDay?.summary?.limitedAvailable ?? 0;
//               const normalCap = availForDay?.summary?.totalNormalCapacity ?? 0;
//               const limitedCap = availForDay?.summary?.totalLimitedCapacity ?? 0;
//               const totalAvail = normalAvail + limitedAvail;
//               const totalCap = normalCap + limitedCap;

//               // Dot colour: green = plenty, amber = tight (<30%), red = full
//               const availRatio = totalCap > 0 ? totalAvail / totalCap : 1;
//               const dotColor =
//                 !hasAvailInfo ? "bg-slate-300"
//                 : allSlotsBooked ? "bg-red-400"
//                 : availRatio < 0.3 ? "bg-amber-400"
//                 : "bg-emerald-400";
...
//                 <div
//                   key={day}
//                   onClick={() => { if (dayIsSelectable) addSessionForDate(day); }}
//                   onMouseEnter={() => setHoveredDay(dateKey)}
//                   onMouseLeave={() => setHoveredDay(null)}
//                   className={`relative h-28 border-b border-r border-slate-100 flex flex-col p-2 transition-all duration-150 select-none
//                     ${countOfThisDate > 0
...
//                     }`}
//                   style={!dayIsSelectable ? { pointerEvents: countOfThisDate > 0 ? "auto" : "none" } : {}}
//                 >
//                   {/* Day number + session badge */}
...
//                     {/* Availability dot */}
//                     {hasAvailInfo && (
//                       <span className={`w-2 h-2 rounded-full mt-1 mr-0.5 ${dotColor}`} />
//                     )}
//                   </div>
...
//                   {countOfThisDate > 0 && (
//                     <div className="mt-1">
//                       <span className="inline-block text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-blue-600 text-white leading-tight">
//                         {countOfThisDate} session{countOfThisDate > 1 ? "s" : ""}
//                       </span>
//                     </div>
//                   )}

//                   {/* Compact availability bar at bottom */}
//                   {hasAvailInfo && !allSlotsBooked && (
...
//                             style={{ width: `${availPct(normalAvail, normalCap)}%` }}
//                           />
//                         </div>
//                         <span className="text-[9px] text-slate-400 w-3 text-right font-mono">{normalAvail}</span>
//                       </div>
//                       {/* Limited slots bar */}
//                       <div className="flex items-center gap-1">
//                         <div className="flex-1 h-1 rounded-full bg-slate-200 overflow-hidden">
...
//                             style={{ width: `${availPct(limitedAvail, limitedCap)}%` }}
//                           />
//                         </div>
//                         <span className="text-[9px] text-amber-500 w-3 text-right font-mono">{limitedAvail}</span>
//                       </div>
//                     </div>
//                   )}

//                   {allSlotsBooked && (
//                     <div className="mt-auto text-[10px] font-semibold text-red-500 tracking-tight">
//                       Full
//                     </div>
//                   )}

//                   {/* ── Hover popover: per-slot breakdown ──────────────────── */}
//                   {isHovered && hasAvailInfo && (
//                     <div
//                       className="absolute z-30 left-0 top-full  w-80 bg-white border border-slate-200 rounded-xl shadow-xl p-3 pointer-events-none"
//                       style={{ minWidth: 240 }}
//                     >
...
//                         <span className="flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full bg-blue-50 border border-blue-200 text-blue-700 font-semibold">
//                           <span className="w-1.5 h-1.5 rounded-full bg-blue-400 inline-block" />
//                           {normalAvail}/{normalCap} normal
//                         </span>
//                         <span className="flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full bg-amber-50 border border-amber-200 text-amber-700 font-semibold">
//                           <span className="w-1.5 h-1.5 rounded-full bg-amber-400 inline-block" />
//                           {limitedAvail}/{limitedCap} limited
//                         </span>
//                       </div>

//                       {/* Per-slot rows — normal first, then limited */}
//                       <div className="space-y-1  overflow-y-auto pr-0.5">
//                         {SESSION_TIME_OPTIONS.map((opt) => {
//                           const slotData = availForDay.slots?.[opt.id];
//                           if (!slotData) return null;
//                           const pct = availPct(slotData.available, slotData.totalCapacity);
//                           const isFull = slotData.available === 0;

//                           // Custom: Show "from - to" format, e.g. "10:00 AM to 10:45 AM"
//                           // opt.label is already "10:00 to 10:45", need to append "AM/PM"
//                           // For best user experience, parse both "from" and "to" times to 12hr with AM/PM

//                           // Helper: Convert "HHMM" (e.g. 1000) to "10:00 AM"
//                           function timeStr24to12(str: string) {
...
//                           // Parse slotId ("1000-1045" → 1000, 1045)
//                           let timeFrom = "", timeTo = "";
//                           if (opt.id && opt.id.includes("-")) {
//                             const [from, to] = opt.id.split("-");
//                             timeFrom = timeStr24to12(from);
//                             timeTo   = timeStr24to12(to);
//                           }

//                           return (
//                             <div key={opt.id} className="flex items-center gap-2">
//                               <span className={`text-[10px] font-mono w-40 shrink-0 ${isFull ? "text-red-400 line-through" : "text-slate-600"}`}>
//                                 {timeFrom && timeTo
//                                   ? `${timeFrom} to ${timeTo}`
//                                   : opt.label}
//                               </span>
...
//                                 />
//                               </div>
//                               <span className={`text-[10px] font-semibold w-8 text-right ${isFull ? "text-red-400" : "text-slate-500"}`}>
//                                 {isFull ? "Full" : `${slotData.available}`}
//                               </span>
//                               {opt.limited && (
...
//           {/* Legend + hint */}
//           <div className="px-5 py-3 border-t border-slate-100 bg-slate-50 flex flex-wrap items-center gap-x-5 gap-y-1.5 text-[11px] text-slate-500">
...
//               <span className="w-8 h-1 rounded-full bg-blue-400 inline-block" /> Normal
//             </span>
//             <span className="flex items-center gap-1.5">
//               <span className="w-8 h-1 rounded-full bg-amber-400 inline-block" /> Limited
//             </span>
//             <span className="ml-auto italic text-slate-400">Hover a day to see slot details</span>
//           </div>

//           {typeof maxSelectableDates === "number" && (
//             <div className="px-5 py-2.5 border-t border-slate-100 text-xs text-slate-600 bg-white">
//               Select up to{" "}
//               <span className="font-semibold text-blue-700">{maxSelectableDates}</span>{" "}
//               session{maxSelectableDates > 1 ? "s" : ""} for this package.
//               At least one is required.
//               {sessions.length >= (maxSelectableDates ?? 0) && (
//                 <span className="ml-2 text-blue-600 font-semibold">Limit reached.</span>
//               )}
//             </div>
//           )}
//         </div>
...
//                 const availSlotsObj = availabilityData[s.date]?.slots || {};
//                 const hasAvailData = Object.keys(availSlotsObj).length > 0;
...
//                         type="button"
//                         title="Remove this session"
//                         className="p-1 rounded-lg border border-red-200 text-red-500 hover:bg-red-50 transition"
//                         onClick={() => removeSessionByIdx(idx)}
//                       >
//                         <FiX size={12} />
//                       </button>
...
//                         value={s.slotId}
//                         onChange={(e) => updateSlotId(idx, e.target.value)}
//                         className={`flex-1 border rounded-lg px-2 py-1.5 text-xs bg-white focus:ring-2 focus:ring-blue-200 outline-none transition ${
//                           idx === 0 && !s.slotId ? "border-red-400" : "border-slate-200"
//                         }`}
//                         required={idx === 0}
//                         aria-required={idx === 0}
//                       >
//                         <option value="">Select Time Slot…</option>
//                         {/* Normal slots group */}
//                         <optgroup label="── Normal Slots ──">
//                           {SESSION_TIME_OPTIONS.filter(o => !o.limited).map((opt) => {
//                             const slotData = availSlotsObj[opt.id];
//                             const available = slotData?.available;
//                             const isFull = available !== undefined && available <= 0;
//                             const isUsed = usedSlots.includes(opt.id);
//                             const suffix = !hasAvailData
//                               ? ""
//                               : isFull
//                               ? " — Full"
//                               : ` — ${available} left`;
//                             return (
//                               <option key={opt.id} value={opt.id} disabled={isUsed || isFull}>
//                                 {opt.label}{suffix}
//                               </option>
//                             );
//                           })}
//                         </optgroup>
//                         {/* Limited slots group */}
//                         <optgroup label="── Limited Case Slots ──">
//                           {SESSION_TIME_OPTIONS.filter(o => o.limited).map((opt) => {
//                             const slotData = availSlotsObj[opt.id];
//                             const available = slotData?.available;
//                             const isFull = available !== undefined && available <= 0;
...
//                     {/* Inline warnings */}
//                     {idx === 0 && !s.slotId && (
//                       <p className="mt-1.5 text-[11px] text-red-500 font-medium">Time required for first session</p>
//                     )}
//                     {s.slotId && availabilityData[s.date]?.slots?.[s.slotId]?.available === 0 && (
//                       <p className="mt-1.5 text-[11px] text-red-500 font-medium">⚠ This slot is no longer available</p>
//                     )}

//                     {/* Mini availability bar for chosen slot */}
//                     {s.slotId && availSlotsObj[s.slotId] && availSlotsObj[s.slotId].available > 0 && (
//                       <div className="mt-2 flex items-center gap-2">
//                         <div className="flex-1 h-1 rounded-full bg-slate-200 overflow-hidden">
//                           <div
//                             className="h-full rounded-full bg-emerald-400 transition-all"
//                             style={{
//                               width: `${availPct(availSlotsObj[s.slotId].available, availSlotsObj[s.slotId].totalCapacity)}%`
//                             }}
//                           />
//                         </div>
//                         <span className="text-[10px] text-emerald-600 font-semibold whitespace-nowrap">
//                           {availSlotsObj[s.slotId].available} of {availSlotsObj[s.slotId].totalCapacity} open
//                         </span>
//                       </div>
//                     )}
//                   </div>
...
//               </div>
//             )}
//           {/* Also show general error if chosen slot(s) is not available */}
//           {!allSlotsAvailable && (
//             <div className="text-xs text-red-600 mt-1">
//               One or more selected slots are not available. Please choose another slot.
//             </div>
//           )}
//         </div>
//       </div>
...
//                                       s.slotId
//                                     )}
//                                     {/* If API info present, show slot filled (0 avail) */}
//                                     {" "}{availabilityData[s.date]?.slots?.[s.slotId] && availabilityData[s.date]?.slots?.[s.slotId].available === 0 && (
//                                       <span className="ml-2 text-xs text-red-700">(Full)</span>
//                                     )}
//                                   </td>
//                                 </tr>
//                               );
...
// type Therapy = { _id: string; name: string }; // removed
type Package = {
  _id: string;
  name: string;
  totalSessions?: number;
...
type BookingSession = { date: string; slotId: string; _id?: string };
// Remove therapy from Booking type, but to maintain legacy type compatibility, leave as-is
type Booking = {
  _id: string;
  requestId?: string;
  appointmentId?: string;
  patient: Patient;
  // therapy: Therapy; // removed
  package: Package | null;
  therapist: Therapist | string;
  sessions: BookingSession[];
  discountInfo?: {
...
export default function RequestAppointment() {
  ...
  const [month, setMonth] = useState<number>(today.getMonth());
  const [patientId, setPatientId] = useState<string>("");
  // const [therapyId, setTherapyId] = useState<string>(""); // removed
  const [packageId, setPackageId] = useState<string>("");
  const [sessions, setSessions] = useState<{ date: string; slotId: string }[]>([]);
  const [selectedCouponId, setSelectedCouponId] = useState<string>("");
  const [coupons, setCoupons] = useState<Coupon[]>([]);
  const [patients, setPatients] = useState<Patient[]>([]);
  // const [therapies, setTherapies] = useState<Therapy[]>([]); // removed
  const [packages, setPackages] = useState<Package[]>([]);
  const [bookings, setBookings] = useState<Booking[]>([]);
  const [dataLoading, setDataLoading] = useState<boolean>(true);
  const [bookingLoading, setBookingLoading] = useState<boolean>(false);
  ...
  useEffect(() => {
    async function fetchMasterDataAndCoupons() {
      ...
      try {
        ...
        setPatients(processedPatients);
        // setTherapies(json.therapyTypes || []); // removed
        setPackages(json.packages || []);
        setCoupons(Array.isArray(json.coupons) ? json.coupons : []);
      } catch {
        setPatients([]);
        // setTherapies([]); // removed
        setPackages([]);
        setCoupons([]);
        toast.error("Failed to load master data");
      }
  ...
  useEffect(() => {
    if (editBookingId) {
      ...
      if (booking) {
        ...
        // setTherapyId(booking.therapy?._id || ""); // removed
        setPackageId(booking.package?._id || "");
        setSessions(
          Array.isArray(booking.sessions)
            ? booking.sessions.map((s) => ({
  ...
  // Removes the last session for the given date (called by the "−" button on the calendar day cell)
  const removeSessionForDate = (dateKey: string) => {
    setSessions((prev) => {
      const idx = prev.map((s) => s.date).lastIndexOf(dateKey);
  ...
  const selectedPatient = patients.find((p) => p.id === patientId) || null;
  // const selectedTherapy = therapies.find((t) => t._id === therapyId) || null; // removed
  const selectedCoupon = coupons.find((c) => c._id === selectedCouponId) || null;
    ...
    >
      <div className="flex md:flex-row flex-col-reverse gap-6">
            ...
            {Array.from({ length: getDaysInMonth(year, month) }).map((_, i) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Session count chip + remove button */}
                  {countOfThisDate > 0 && (
                    <div className="mt-1 flex items-center gap-1">
                      <span className="inline-block text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-blue-600 text-white leading-tight">
                        {countOfThisDate} session{countOfThisDate > 1 ? "s" : ""}
                      </span>
                      {/* "−" button: removes one session for this date, same as the appointment booking calendar */}
                      <button
                        type="button"
                        title="Remove one session for this date"
                        onClick={(e) => {
                          e.stopPropagation();
                          removeSessionForDate(dateKey);
                        }}
                        className="inline-flex items-center justify-center w-4 h-4 rounded-full bg-red-100 text-red-600 hover:bg-red-200 border border-red-300 font-bold text-xs leading-none transition"
        ...
        <div className="flex-1 bg-white border rounded-lg p-6">
          ...
          {/* Therapy type selection removed */}
          ...
          <div className="flex flex-col md:flex-row md:items-end gap-2 md:gap-6 mb-4">
            <div className="flex flex-row items-center gap-2">
              ...
              <input
                type="text"
                value={reqSearchImmediate}
                placeholder="Search by request id / children / status / coupon / date"
                className="px-2 py-1 border rounded text-sm w-64"
                onChange={e => setReqSearchImmediate(e.target.value)}
                spellCheck={false}
              />
