...
const RaiseTicketsParent: React.FC = () => {
  ...
  function getParentAuthToken() {
    return localStorage.getItem("patient-token");
  }
  ...
  async function fetchTickets(pg = 1, limit = 20) {
    ...
    try {
      ...
      const res = await axios.get(
        `${apiUrl}/api/parent/tickets?page=${pg}&limit=${limit}`,
        {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await axios.post(
        `${apiUrl}/api/parent/ticket/raise`,
        ticketPayload,
        {
          headers: token
            ? { Authorization: `${token}` }
  ...
  // --- Helper: Render ticket action bar (this is removed - no more replies) ---
//   function renderReplyBox(ticket: any) {
//     // Feature to send another response is removed
//     return null;
//   }
...
export default RaiseTicketsParent;