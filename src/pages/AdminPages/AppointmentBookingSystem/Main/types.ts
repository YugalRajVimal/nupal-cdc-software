...
/**
 * Returns which occurrence of its weekday a given day falls on within its
 * month — e.g. day 9 (a Monday) is the 2nd Monday of that month.
 * 1-indexed (1st, 2nd, 3rd, ...).
 */
export function getWeekdayOccurrenceInMonth(day: number): number {
  return Math.ceil(day / 7);
}
 
/**
 * Finds the day-of-month for the Nth occurrence of `dayOfWeek` (0=Sun..6=Sat)
 * in the given target month/year. Returns `null` if that occurrence doesn't
 * exist in the target month (e.g. asking for the 5th Monday when the month
 * only has 4).
 */
export function findNthWeekdayInMonth(
  year: number,
  month0: number, // 0-indexed
  dayOfWeek: number, // 0=Sun..6=Sat
  occurrence: number // 1-indexed
): number | null {
  const daysInTargetMonth = getDaysInMonth(year, month0);
  let seen = 0;
  for (let day = 1; day <= daysInTargetMonth; day++) {
    if (new Date(year, month0, day).getDay() === dayOfWeek) {
      seen++;
      if (seen === occurrence) return day;
    }
  }
  return null;
}
 ...
 * so the earliest original session lands in `targetMonth`/`targetYear`, with
 * every other session shifted by that same number of months — same
 * day-of-week + same "week of the month" (e.g. a session on the 1st Monday
 * of the source month lands on the 1st Monday of the target month), same
 * time slot, same therapist, same therapy type. Used by the "Copy booking →
 * Paste into another month" feature so Admin doesn't have to re-enter an
 * identical booking for a later month.
 *
 * If a session's original weekday-occurrence doesn't exist in its shifted
 * target month (e.g. a 5th Monday shifted into a month with only 4 Mondays),
 * that session's `date` is left as an empty string and its index (within the
 * input array) is returned in `needsFixIndices`, so the UI can flag it for
 * Admin to pick a date manually instead of silently guessing.
 */
export function shiftSessionsToMonth(
  sessions: BookingSession[],
  targetMonth: number, // 0-indexed (Jan = 0), matches the calendar's `month` state
  ...
  const shifted = (sessions || []).map((s, idx) => {
    if (!s?.date) return { ...s };
    const [origYear, origMonth1, origDay] = s.date.split("-").map(Number);
    const origMonth0 = origMonth1 - 1;
    const rawMonthIndex = origMonth0 + monthOffset;
    const newYear = origYear + Math.floor(rawMonthIndex / 12);
    const newMonth0 = ((rawMonthIndex % 12) + 12) % 12;
 
    const dayOfWeek = new Date(origYear, origMonth0, origDay).getDay();
    const occurrence = getWeekdayOccurrenceInMonth(origDay);
    const newDay = findNthWeekdayInMonth(newYear, newMonth0, dayOfWeek, occurrence);
 
    if (newDay === null) {
      needsFixIndices.push(idx);
      return { ...s, date: "" };
    }
    return { ...s, date: getDateKey(newYear, newMonth0 + 1, newDay) };
  });
