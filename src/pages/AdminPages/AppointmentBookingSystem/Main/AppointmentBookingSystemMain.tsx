...
//   // of every pasted session whose original weekday-occurrence (e.g. "2nd
//   // Monday") didn't exist in the target month — flagged in the form so
//   // Admin can pick a date manually.
...
//    * a "paste into another month" whose original weekday-occurrence didn't
//    * exist in the target month (see shiftSessionsToMonth / dateFixSessionIds).
//    */
...
//   /**
//    * Row-by-row explanation of what's blocking Book Now, instead of a single
//    * generic message. Session numbers are 1-indexed against `sessions` in its
//    * current (date-sorted) order, matching what Admin sees in the table. Used
//    * so that after a paste — where the button now always stays enabled — the
//    * error clicking through surfaces exactly which row(s)/field(s) still need
//    * attention rather than just refusing silently.
//    */
//   function getBookingValidationIssues(): string[] {
//     const issues: string[] = [];
//     if (!selectedPatient) issues.push("Select a patient.");
//     if (!selectedPackage) issues.push("Select a package.");
//     if (sessions.length === 0) { issues.push("Add at least one session."); return issues; }
 
//     sessions.forEach((s, idx) => {
//       const rowLabel = `Session ${idx + 1}${s.date ? ` (${s.date})` : ""}`;
//       if (dateFixSessionIds.has(s.sessionId)) {
//         issues.push(`${rowLabel}: pick a date manually — it couldn't be pasted onto a matching day in this month.`);
//         return; // other checks on this row are moot until the date is set
//       }
//       if (!s.date) issues.push(`${rowLabel}: missing date.`);
//       if (!s.slotId) issues.push(`${rowLabel}: missing time slot.`);
//       const hasTherapyType =
...
//       if (!hasTherapyType) issues.push(`${rowLabel}: missing therapy type.`);
//       if (!s.therapistId) issues.push(`${rowLabel}: missing therapist.`);
//     });
 
//     sessions.forEach((s, idx) => {
//       if (!s.date || !s.slotId) return;
//       const dupIdx = sessions.findIndex((os, oi) => oi !== idx && os.date === s.date && os.slotId === s.slotId);
//       if (dupIdx !== -1 && dupIdx > idx) {
//         issues.push(`Session ${idx + 1} and Session ${dupIdx + 1} both use ${s.date} at the same time slot — pick different slots.`);
//       }
//     });
...
//   const bookingValidationIssues = getBookingValidationIssues();
//   const canBook = bookingValidationIssues.length === 0;
...
//     if (issues.length > 0) {
//       setBookingError(issues.join(" \u2022 "));
//       toast.error(
//         issues.length === 1 ? issues[0] : `${issues.length} things need attention — see details below the form.`,
//         { autoClose: 5000 }
//       );
//       return;
//     }
...
//    * session lands in targetMonth/targetYear — same day-of-week + same
//    * "week of the month" (e.g. 1st Monday → 1st Monday), same time slot,
//    * therapist, therapy type, patient, package. Sessions always start unattended
...
//     // ── Surface what's missing right away ────────────────────────────────
//     // Book Now stays enabled after a paste (Admin may still want to open the
//     // form and fix things manually), but we tell them up front what needs
//     // attention instead of leaving it to a failed click. Computed from the
//     // just-built local values since the state setters above haven't
//     // committed yet in this render.
//     const pastedPackage = packages.find((p) => p._id === (copiedBooking.package as any)?._id) || null;
//     const pasteIssues: string[] = [];
//     if (!foundPatient) pasteIssues.push("Patient couldn't be matched — select one manually.");
//     if (!pastedPackage) pasteIssues.push("Package couldn't be matched — select one manually.");
//     sortedShifted.forEach((s, idx) => {
//       const rowLabel = `Session ${idx + 1}${s.date ? ` (${s.date})` : ""}`;
//       if (needsFixSet.has(s.sessionId)) {
//         pasteIssues.push(`${rowLabel}: pick a date manually — it couldn't be pasted onto a matching day in this month.`);
//         return;
//       }
//       if (!s.slotId) pasteIssues.push(`${rowLabel}: missing time slot.`);
//       if (!s.therapistId) pasteIssues.push(`${rowLabel}: missing therapist.`);
//       const hasTherapyType =
//         (typeof s.therapyTypeId === "object" && s.therapyTypeId && (s.therapyTypeId as any)._id && (s.therapyTypeId as any).name) ||
//         (typeof s.therapyTypeId === "string" && s.therapyTypeId);
//       if (!hasTherapyType) pasteIssues.push(`${rowLabel}: missing therapy type.`);
//     });
//     sortedShifted.forEach((s, idx) => {
//       if (!s.date || !s.slotId) return;
//       const dupIdx = sortedShifted.findIndex((os, oi) => oi !== idx && os.date === s.date && os.slotId === s.slotId);
//       if (dupIdx !== -1 && dupIdx > idx) {
//         pasteIssues.push(`Session ${idx + 1} and Session ${dupIdx + 1} both use ${s.date} at the same time slot — pick different slots.`);
//       }
//     });
//     setBookingError(pasteIssues.length > 0 ? pasteIssues.join(" \u2022 ") : null);
...
//         { autoClose: 6000 }
//       );
//     } else if (pasteIssues.length > 0) {
//       toast.warn(`Pasted — ${pasteIssues.length} thing(s) need attention before you can book. See details below the form.`, { autoClose: 5000 });
//     } else {
      ...
      if (editingBooking) {
        for (const s of editingBooking.sessions || []) {
          ...
          if (
            ...
            // Don't exclude this slot just because it USED to belong to this
            // booking — if a *different* booking's session also sits in this
            // exact date/slot/therapist (e.g. this booking's old session was
            // marked Missed and the slot got picked up by another patient),
            // it's still genuinely taken and must keep showing as booked.
            const heldByOtherBooking = bookings.some((other) => {
              if (other._id === editBookingId) return false;
              return (other.sessions || []).some((os) => {
                const osTherapistId =
                  os.therapistId ||
                  (os.therapist && typeof os.therapist === "object" ? (os.therapist as any)._id : "") ||
                  (typeof os.therapist === "string" ? os.therapist : "");
                return os.date === apiDate && os.slotId === s.slotId && osTherapistId === tId;
              });
            });
            if (!heldByOtherBooking) {
              ownSlotsCalendar.add(s.slotId);
            }
          }
        }
      ...
      if (editingBooking) {
        ...
        for (const s of editingBooking.sessions || []) {
          ...
          if (
            ...
            // Same fix as getTherapistAvailableSlotsForDay: only treat this
            // slot as "safe to keep / not a conflict" when it's exclusively
            // this booking's own slot. If another booking's session also
            // occupies this exact date/slot/therapist (e.g. this booking's
            // original session here was later marked Missed and a different
            // patient booked into the now-free-looking slot), it's a real,
            // live conflict and must still be flagged "Already booked".
            const heldByOtherBooking = bookings.some((other) => {
              if (other._id === editBookingId) return false;
              return (other.sessions || []).some((os) => {
                const osTherapistId =
                  ...
                  (os.therapist && typeof os.therapist === "object" ? (os.therapist as any)._id : "") ||
                  (typeof os.therapist === "string" ? os.therapist : "");
                return os.date === apiDate && os.slotId === s.slotId && osTherapistId === rowTherapistId;
              });
            });
            if (!heldByOtherBooking) {
              ownSlots.add(s.slotId);
