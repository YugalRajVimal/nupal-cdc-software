...
import {
  ...
  formatDateDDMMYYYY, getWeekdayName, getTotalSessionsForPackage, getPatientDisplayName, getPackageDisplay,
} from "./types";
...
export function BookingFormPanel({
  editBookingId, handleReset, handleCancelEdit, handleBookOrUpdate, bookingLoading, bookingError, bookingSuccess,
    ...
    <div className="flex-1 bg-white border rounded-lg p-6">
      ...
      {bookingError && (
        <ul className="text-xs text-red-600 mt-1 list-disc pl-4 space-y-0.5">
          {bookingError.split(" \u2022 ").filter(Boolean).map((issue, i) => (
            <li key={i}>{issue}</li>
          ))}
        </ul>
      )}
