...
export default function PatientRegistration() {
  ...
  // Synchronous lock — prevents double-submit from rapid/double clicks.
  // React state updates (setSubmitting) are batched/async, so relying on
  // `submitting` alone can let two clicks both pass the check before the
  // first re-render lands. A ref is read/written synchronously.
  ...
  const submit = async () => {
    // Bail immediately if a submission is already in flight.
    ...
    try {
      ...
      // Always release the lock, whether the request succeeded, failed,
      // or threw — otherwise a network error would permanently disable
      // the Submit button for the rest of the session.
