          ...
          <div className="bg-white border rounded-lg overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {financeData.logs && financeData.logs.length > 0 ? (
                  (() => {
                    // Group rows that came from ONE payment collection action
                    // (a booking's own portion + any "cleared another booking's
                    // due" rows) so we can show the split as one transaction
                    // instead of a handful of unrelated-looking income rows.
                    ...
                    return logs.flatMap((log, idx) => {
                      ...
                      const rows = [
                        ...
                        <tr
                          key={idx}
                          className={`border-t ${isSplit ? "bg-amber-50/60" : ""}`}
                        >
                          <td className="px-4 py-3">
                            {log.Date
                              ? (() => {
                      ...
                      // Right after the FIRST row of a split, show a one-line
                      // breakdown of the whole group: total collected and how
                      // much went to each booking.
                          ...
                          <tr key={`${idx}-split-summary`} className="bg-amber-50/60">
                            ...
                            <td colSpan={10} className="px-4 pb-3 pt-0">
                              <div className="text-xs text-amber-800 flex flex-wrap gap-x-4 gap-y-1">
                                ...
                                {group.map((r, gi) => (
                                  <span key={gi}>
                                    ₹{Number(r.Amount).toLocaleString("en-IN")} — {r.Description}
                                  </span>
                                ))}
                              </div>
                            </td>
