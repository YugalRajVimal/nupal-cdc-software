...
const AppointmentCard = ({
  ...
  // ── Payment details, computed the same way as BookingSummary ──────────────
  const invoiceAmount = booking.invoiceAmount ?? 0;
  const paidAmount = booking.payment?.amountPaid ?? booking.payment?.amount ?? 0;
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 px-6 py-6">
          ...
          <details open className="group">
            ...
            {/* ── Payment block — mirrors BookingSummary's invoice/paid/due/wallet layout ── */}
            <div className="mt-2 px-2 py-2 rounded bg-sky-100 border border-sky-200 text-xs space-y-1 w-fit min-w-[260px]">
              <div className="flex items-center justify-between font-semibold text-blue-900 gap-6">
                <span>Sessions checked in</span>
                <span className="font-mono">{checkedInCount} / {totalSessions}</span>
              </div>
              {discountPercent > 0 && (
                <div className="flex items-center justify-between text-green-700 font-semibold gap-6">
                  <span>
              ...
              <div className="flex items-center justify-between text-blue-900 font-semibold gap-6">
                <span>Current invoice amount</span>
                <span className="font-mono">₹{invoiceAmount}</span>
              </div>
              <div className="flex items-center justify-between text-slate-700 gap-6">
                <span>Paid so far</span>
                <span className="font-mono">₹{paidAmount}</span>
              </div>
              <div className="flex items-center justify-between text-rose-700 font-semibold border-t border-sky-200 pt-1 mt-1 gap-6">
                <span>Due now</span>
                <span className="font-mono">₹{dueNow}</span>
              </div>
              <div className="flex items-center justify-between border-t border-sky-200 pt-1 mt-1 gap-6">
                <span className="text-emerald-700 font-medium flex items-center gap-1">
                  <FiPocket size={12} /> Wallet balance
                </span>
                <span className="font-bold text-emerald-700">₹{walletBalance}</span>
              </div>
              <div className="flex items-center gap-2 border-t border-sky-200 pt-1 mt-1">
              ...
              {booking.payment && (
                <div className="border-t border-sky-200 pt-1 mt-1 text-slate-500">
                  <span className="text-xs">
                    Last payment: ₹{booking.payment.amount}
                    {booking.payment.paymentMethod && ` (${booking.payment.paymentMethod})`}
                  </span>
                  {booking.payment.paymentId && (
                    <div className="text-xs font-mono">ID: {booking.payment.paymentId}</div>
                  )}
                </div>
              )}
            </div>
