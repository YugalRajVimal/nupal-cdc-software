...
const CompleteProfilePage: React.FC = () => {
  ...
  const [step, setStep] = useState<number>(1);
  const [docs, setDocs] = useState<DocumentsState>(initialDocs);
  const [fileErrors, setFileErrors] = useState<Partial<Record<keyof DocumentsState, string>>>({});
  const [isSubmitting, setIsSubmitting] = useState(false);
  ...
  // Scroll to top whenever the step changes — important on mobile where
  // the form can be much taller than the viewport.
  React.useEffect(() => {
  ...
  function updateDoc(key: keyof DocumentsState, file: File | null) {
    if (file) {
      ...
      if (!okType) {
        setFileErrors((p) => ({ ...p, [key]: "Only PDF, JPG or PNG allowed" }));
        toast.error("Only PDF, JPG or PNG files are allowed.", { position: "top-center" });
        return;
      }
      if (file.size > MAX_FILE_SIZE) {
        setFileErrors((p) => ({ ...p, [key]: "File must be under 5MB" }));
        toast.error("File is too large. Max size is 5MB.", { position: "top-center" });
        return;
      }
    }
    setFileErrors((p) => {
  ...
  async function next() {
    ...
    if (step === 2) {
      const anyMissing = FILE_FIELDS.some((f) => !isValidUploadedFile(docs[f]));
      const anyInvalid = Object.keys(fileErrors).length > 0;
      if (anyMissing || anyInvalid) {
        toast.error("Please upload all required documents correctly.", { position: "top-center" });
        return;
      }
      setStep(3);
      return;
  ...
  function goToStep(target: number) {
    // Allow jumping backward freely, but forward only through validation.
    if (target < step) {
      setStep(target);
    }
  }
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-tr from-violet-200 via-indigo-200 to-amber-100">
      <ToastContainer />
      <div className="w-full max-w-5xl mx-auto flex flex-col items-center py-6 px-3 sm:py-10 sm:px-6 lg:px-8 pb-28 sm:pb-32">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold leading-tight text-center mb-6 sm:mb-10 text-indigo-900 drop-shadow-md tracking-tight">
          Therapist Profile Completion
        </h1>

        {/* Stepper */}
        <MobileStepper step={step} onStepClick={goToStep} />
        <div className="hidden sm:flex w-full max-w-3xl mx-auto items-center justify-between mb-10">
          {steps.map((label, i) => {
            const idx = i + 1;
            ...
            return (
              <div key={label} className="flex-1 flex items-center">
                <button
                  ...
                  className={`h-11 w-11 md:h-12 md:w-12 shrink-0 shadow-lg bg-white/60 backdrop-blur border-2 transition flex items-center justify-center text-lg md:text-xl font-bold rounded-full ${
                    step >= idx
                      ? "border-blue-600 text-blue-700 ring-2 ring-blue-400"
                ...
                <span
                  className={`ml-3 md:ml-4 text-sm md:text-base font-semibold whitespace-nowrap ${
                    step >= idx ? "text-indigo-900" : "text-gray-400"
                  }`}
                >
                  {label}
                </span>
                {i < steps.length - 1 && (
                  <div className="flex-1 h-0.5 bg-gradient-to-r from-blue-200 to-indigo-300 mx-2 md:mx-3 opacity-80" />
                )}
              </div>
            );
          })}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {step === 1 && (
                <Section title="Personal Information">
                  ...
                  <Input
                    ...
                    type="tel"
                    inputMode="tel"
                    error={errors.mobile2?.message}
                    {...register("mobile2")}
                  />
                  <Input label="Full Address *" error={errors.address?.message} {...register("address")} full />
                  <Input label="Reference *" error={errors.reference?.message} {...register("reference")} />
                  <Input label="Specializations *" error={errors.specializations?.message} {...register("specializations")} />
                  <Input
              ...
              {step === 2 && (
                <Section title="Document Upload">
                  ...
                  <FilePicker
                    ...
                    onChange={(f) => updateDoc("certificate", f)}
                  />
                  <p className="md:col-span-2 text-xs text-indigo-600/80 -mt-2">
              ...
              {step === 4 && (
                <div className="space-y-6">
                  <Section2 title="Review & Submit">
                    <div className="bg-white/40 backdrop-blur-lg p-4 sm:p-6 rounded-2xl border border-indigo-200 grid grid-cols-1 md:grid-cols-2 gap-6 text-sm sm:text-base transition drop-shadow">
                      <ReviewBlock title="Personal" onEdit={() => goToStep(1)}>
                        <Detail label="Full Name" value={getValues("fullName")} />
                        <Detail label="Email" value={getValues("email")} />
                        <Detail label="Father's Name" value={getValues("fathersName")} />
                        <Detail label="Mobile 1" value={getValues("mobile1")} />
                      ...
                      <ReviewBlock title="Bank" onEdit={() => goToStep(3)}>
                        <Detail label="Account Holder" value={getValues("accountHolder")} />
                        <Detail label="Bank Name" value={getValues("bankName")} />
                        <Detail label="IFSC" value={getValues("ifsc")} />
                        <Detail label="Account No." value={getValues("accountNumber")} />
                      ...
                      <ReviewBlock title="Documents" onEdit={() => goToStep(2)}>
                        <Detail label="Aadhaar Front" value={docs.aadhaarFront?.name || "Not uploaded"} />
                        <Detail label="Aadhaar Back" value={docs.aadhaarBack?.name || "Not uploaded"} />
                        <Detail label="Photo" value={docs.photo?.name || "Not uploaded"} />
                        <Detail label="Resume" value={docs.resume?.name || "Not uploaded"} />
                      ...
                      <ReviewBlock title="Links" onEdit={() => goToStep(3)}>
                        <Detail label="LinkedIn" value={getValues("linkedin") || "-"} />
                        <Detail label="Twitter" value={getValues("twitter") || "-"} />
                        <Detail label="Facebook" value={getValues("facebook") || "-"} />
                        <Detail label="Instagram" value={getValues("instagram") || "-"} />
        ...
        <div className="mt-8 sm:mt-10 flex justify-center w-full">
          <button
            type="button"
            className="px-6 py-2.5 rounded-lg bg-indigo-700 text-white shadow-lg hover:bg-indigo-800 font-semibold transition text-sm sm:text-base"
            onClick={() => (window.location.href = "/signin")}
          >
            Back to Sign In
          </button>
      ...
      {/* Sticky action bar so buttons are always reachable, even with a tall form on mobile */}
      <div className="fixed bottom-0 left-0 right-0 z-20 bg-white/80 backdrop-blur-md border-t border-indigo-200 shadow-[0_-4px_16px_rgba(79,70,229,0.08)]">
        <div className="w-full max-w-5xl mx-auto flex flex-row justify-between items-center gap-3 px-3 sm:px-8 py-3">
          <button
            type="button"
            disabled={step === 1 || isSubmitting}
            onClick={previous}
            className="flex-1 sm:flex-none px-4 sm:px-6 py-3 rounded-lg bg-white border border-indigo-300 shadow hover:bg-blue-50 transition font-semibold text-indigo-900 disabled:opacity-40 disabled:cursor-not-allowed"
          >
            Previous
          </button>
          {step < 4 ? (
            <button
              ...
              onClick={next}
              disabled={isSubmitting}
              className="flex-1 sm:flex-none px-6 sm:px-8 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg shadow-lg font-semibold hover:scale-[1.02] active:scale-95 transform transition disabled:opacity-40"
            >
              Next
            </button>
          ) : (
            <button
              ...
              onClick={submit}
              disabled={isSubmitting}
              className="flex-1 sm:flex-none px-6 sm:px-8 py-3 bg-gradient-to-r from-emerald-500 to-green-600 text-white rounded-lg shadow-lg font-semibold hover:scale-[1.02] active:scale-95 transform transition disabled:opacity-40 flex items-center justify-center gap-2"
            >
              {isSubmitting && (
...
function MobileStepper({ step, onStepClick }: { step: number; onStepClick: (n: number) => void }) {
  const pct = ((step - 1) / (steps.length - 1)) * 100;
  return (
    <div className="sm:hidden w-full mb-6">
      <div className="flex items-center justify-between mb-2">
        <span className="text-sm font-bold text-indigo-900">{steps[step - 1]}</span>
          ...
          return (
            <button
              ...
              className={`text-[11px] font-semibold ${
                step >= idx ? "text-indigo-800" : "text-gray-400"
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-8 sm:mb-10 w-full">
      <h2 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 text-indigo-800 tracking-tight">{title}</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 bg-white/30 rounded-2xl p-4 sm:p-6 shadow transition border border-indigo-100">
        {children}
      </div>
    </div>
  );
...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  ({ label, error, full, className, ...props }, ref) => (
    <div className={`flex flex-col mb-1 relative ${full ? "md:col-span-2" : ""}`}>
      <label className="text-sm text-indigo-700 font-semibold mb-1">{label}</label>
      <input
        ref={ref}
        {...props}
        className={`p-3 border rounded-lg focus:ring-2 focus:ring-indigo-400 outline-none bg-white/70 font-medium placeholder-gray-400 text-base disabled:opacity-60 disabled:cursor-not-allowed ${
          error ? "border-red-500" : "border-indigo-200"
...
function FilePicker({
  ...
  const inputId = useMemo(() => `file-${label.replace(/\W+/g, "-")}`, [label]);
  ...
  return (
    <div className="flex flex-col gap-1">
      <label className="text-sm font-bold text-indigo-800" htmlFor={inputId}>
        {label}
      </label>
      <label
        htmlFor={inputId}
        ...
        onDrop={handleDrop}
        className={`border-2 border-dashed rounded-xl p-4 bg-white/40 shadow transition cursor-pointer block ${
          error
            ? "border-red-500 ring-2 ring-red-200"
            : isDragging
            ? "border-blue-500 ring-2 ring-blue-200 bg-blue-50/50"
            : "border-indigo-200 hover:border-blue-400"
        }`}
      >
        <input
          id={inputId}
          ...
          accept=".pdf,.jpg,.jpeg,.png"
          onChange={handle}
          className="text-sm bg-transparent w-full file:mr-3 file:py-2 file:px-3 file:rounded-lg file:border-0 file:bg-indigo-100 file:text-indigo-700 file:font-semibold file:text-xs hover:file:bg-indigo-200"
        />
        <p className="mt-2 text-[11px] text-indigo-500 sm:hidden">Tap to browse, or drag a file here on desktop</p>
        {file ? (
          <div className="mt-3 flex items-center gap-3 bg-indigo-50/80 rounded-lg px-3 py-2">
            {previewUrl ? (
              <img
                src={previewUrl}
                alt={`${label} preview`}
                className="h-10 w-10 rounded object-cover border border-indigo-200 shrink-0"
              />
            ) : (
              <div className="h-10 w-10 rounded bg-indigo-100 flex items-center justify-center text-indigo-500 text-xs font-bold shrink-0">
                PDF
              </div>
            )}
            <div className="min-w-0 flex-1 text-xs text-indigo-700">
              <p className="truncate font-medium">{file.name}</p>
              <p className="text-indigo-400">{formatBytes(file.size)}</p>
            </div>
            <button
              type="button"
              className="text-red-600 font-semibold hover:underline text-xs shrink-0"
              onClick={(e) => {
                e.preventDefault();
                onChange(null);
              }}
...
function Detail({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-center mb-2 gap-0.5 sm:gap-0">
      <span className="font-medium text-indigo-900 sm:w-40 shrink-0">{label}:</span>
      <span className="text-indigo-950 break-words">{value}</span>
    </div>
  );
}
